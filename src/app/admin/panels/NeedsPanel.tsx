      ...
      <section className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">
            Blood alerts. Only an approved hospital can post one, and it reaches every registered
            blood donor. Taking one down does not tell the people who said available — ring them.
          </p>
        ...
        {isLoading ? (
          ...
          status === 'open' ? (
            <EmptyState
              ...
              title="Nothing open"
              hint="No hospital has an open blood alert right now."
            />
          ) : (
            <EmptyState
            ...
            {requests.map((need) => (
              ...
              >
                <Field label="Answered">
                  {need.responses_count} available of {need.donors_needed} units ·{' '}
                  {need.not_available_count} not available
                </Field>
                <Field label="Area">{need.pincode ?? '—'}</Field>
